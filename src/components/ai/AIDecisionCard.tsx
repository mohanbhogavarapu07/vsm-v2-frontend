    ...
    <div className="rounded-lg border border-border bg-card p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          {decision.task_title && (
            <p className="mt-0.5 text-xs text-muted-foreground">Task: {decision.task_title}</p>
          )}
                ...
                <div className="mt-1 flex items-center gap-2">
                  <div className="h-1.5 flex-1 rounded-full bg-border">
                    <div
                      className={cn(
                        'h-1.5 rounded-full',
                        decision.confidence > 0.85 ? 'bg-success' : decision.confidence > 0.6 ? 'bg-warning' : 'bg-destructive'
                      )}
                      style={{ width: `${decision.confidence * 100}%` }}
                    />
                  </div>
                  <span className="text-xs font-semibold">{Math.round(decision.confidence * 100)}%</span>
                </div>
              </div>
